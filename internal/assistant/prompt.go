...
2. ` + "`evidence`" + ` empty → the bank holds nothing on that point. Call ` + "`request_confirmation`" + ` with the exact claim text and a short question — do NOT write the ask as free-text prose, the candidate sees it as a claim with Да/Нет buttons and Да replays your claim text verbatim as their next message. If they confirm, record it with ` + "`experience_add`" + ` — putting their own words in ` + "`said`" + ` — and then write the bullet citing the id it returns. If they decline, leave it out; a gap belongs in a cover letter, never keyword-stuffed into a CV.
